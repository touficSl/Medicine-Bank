...
class CallsController extends VoyagerBaseController
{
    use BreadRelationshipParser;

    public function pending_pickups (Request $request)
    {
        // GET THE SLUG, ex. 'posts', 'pages', etc.
        $slug = 'suggested-orders';

        // GET THE DataType based on the slug
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();

        // Check permission
        $this->checkPermission('browse_calls');
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope))) {
                ...
                $query = $model::select('*')->where(['status' => 3]);
            }
        ...
        if (!empty($dataTypeContent->first())) {
            foreach (Voyager::actions() as $action) {
                if ($action == "TCG\Voyager\Actions\DeleteAction") {
                    // Do not allow deleting an order.
                    continue;
                }
        ...
        $pending_sales =  DB::table('suggested_orders')
                            ->join('clients', 'clients.id', '=', 'suggested_orders.client_id')
                            ->join('pharmacies', 'pharmacies.id', '=', 'clients.pharmacy_id')
                            ->join('medications as med', 'suggested_orders.medication_id', '=', 'med.id')
                            ->select(DB::raw('suggested_orders.id, CONCAT(clients.first_name_ar, " ", clients.father_name_ar, " ", clients.last_name_ar) as `Client`, suggested_orders.quantity, pharmacies.name_ar AS `Pharmacy`,
                                CONCAT(med.brand, " ", med.dose_per_tablet, " x ", med.quantity_of_tablets) AS `medication`'))
                            ->where(['suggested_orders.status' => 3])
                            ->get();

        $view = 'voyager::calls.pending';
    ...
    public function index(Request $request)
    {
        ...
        $request->session()->put('sugg_order_id', $request->get('id'));
        $sugg_order_id = $request->get('id');
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope))) {
                ...
                $query = $model::select('*')->where(['suggested_order_id' => $sugg_order_id]);
            }
        ...
        if (!empty($dataTypeContent->first())) {
            foreach (Voyager::actions() as $action) {
                if ($action == "TCG\Voyager\Actions\ViewAction" || $action == "TCG\Voyager\Actions\DeleteAction") {
                    // Do not allow viewing or deleting.
                    continue;
                }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (auth()->user()->can('browse', $model)) {
            $redirect = redirect()->route("voyager.{$dataType->slug}.index", 'id='.$request->session()->get('sugg_order_id', 'xx'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        $sugg_order_id = $request->session()->get('sugg_order_id', 'xx');

        // Validate fields with ajax
        $val = $this->validateBread($request->all(), $dataType->addRows)->validate();
        // $data = $this->insertUpdateData($request, $slug, $dataType->addRows, new $dataType->model_name());

        $old_calls = \App\Call::select(DB::raw('count(id) as call_numb'))->where(['suggested_order_id' => $sugg_order_id])
                                ->first();

        $new_call = new \App\Call;
        $new_call->user_id = Auth::user()->id;
        $new_call->suggested_order_id = $sugg_order_id;
        $new_call->answered = $request->get('answered');
        $new_call->comments = $request->get('comments');
        $new_call->call_number = $old_calls['call_numb'] + 1;

        $new_call->save();

        $redirect = redirect()->route("voyager.{$dataType->slug}.index", 'id=' . $sugg_order_id);
