...
class OrdersController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope))) {
                ...
                if ($request->get('ordersType') == 'active') {
                    // Status is 0 = "Placed", 1 = "Confirmed", or 2 = "Delivered"
                    $query = $model::select('*')->where('status', '=', 0)->where('status', '=', 1)->where('status', '=', 2);
                } elseif ($request->get('ordersType') == 'closed') {
                    // Status is 3 = "Closed" or 4 = "Canceled"
                    $query = $model::select('*')->where('status', '=', 3)->where('status', '=', 4);
                } else {
                    $query = $model::select('*');
                }
            }
    ...
    public function show(Request $request, $id)
    {
        ...
        $orderSummary = SuggestedOrdersController::getOrderSummary($dataTypeContent->id);

        // Gather the client information.
        $orders = $this->getSuggestedOrders($dataTypeContent->id);
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted', 'orderSummary', 'orders'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'orderSummary', 'orders'));
    }

    private function getSuggestedOrders($orderID) {
        return DB::table('suggested_orders')
            ...
            ->select(DB::raw('CONCAT(clients.first_name_ar, " ", clients.father_name_ar, " ", clients.last_name_ar) client, CONCAT(medications.brand, " ", medications.dose_per_tablet, " x ", medications.quantity_of_tablets) AS `medication`, suggested_orders.quantity, pharmacies.name_ar AS `pharmacy`'))
    ...
    public function update(Request $request, $id)
    {
        ...
        $isSendBackToClosedOrders = ($request->get('currentStatus') == 4);
        if ($request->get('status') == 4) {
            $emails = $request->get('cancelOrderEmails');
            if (isset($emails) && !is_null($emails) && $emails != "") {
                $emails = explode(",", $emails);
            }
            $dataTypeContent = DB::table($dataType->name)->where('id', $id)->first();
            $this->sendCancelOrderEmail($dataTypeContent, $emails);
        }
        ...
        if (auth()->user()->can('browse', $model)) {
            $redirect = redirect()->route("voyager.{$dataType->slug}.index", ["ordersType" => (($isSendBackToClosedOrders) ? "closed" : "active")]);
        } else {
    ...
    private function sendCancelOrderEmail($order, $emails) {
        ...
        //$from = "MB-Jordan";
        $subject = "MB-Jordan CANCEL ORDER";
        $message = "
        <html>
        <head>
        <title>MB-Jordan CANCEL ORDER</title>
        </head>
        <body>
        <p>
