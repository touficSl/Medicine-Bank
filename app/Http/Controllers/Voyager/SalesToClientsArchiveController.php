...
class SalesToClientsArchiveController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope))) {
                ...
                // select all rows where order status = delivered and sales to client status = pickup pending 
                $query = $model::select('suggested_orders.*')
                                ->distinct('client_id')
                                ->distinct('timestamp_sold')
